...
def _build_execution_cofig(env_name, extra_env_vars=None):
    ...
    for p in aws_ssm_parameters:
        response = ssm_client.get_parameter(Name=f"/{env_name}/{p}")
        config[p] = response["Parameter"]["Value"]

    # Networking config
    response = ssm_client.get_parameter(Name=f"/{env_name}/VpcPrivateSubnetsParam")
    ...
    config["environment"] = [
        # Regular parameters
        {"name": "DJANGO_SETTINGS_MODULE", "value": "app.settings.stage"},
    ...
    response = ssm_client.get_parameter(Name=f"/{env_name}/StaticFilesBucketNameParam")
    ...
    response = ssm_client.get_parameter(Name=f"/{env_name}/SqsDefaultQueueUrlParam")
    config["environment"].append(
        {"name": "SQS_DEFAULT_QUEUE_URL", "value": response["Parameter"]["Value"]}
    )

    # Retrieve secret values from secrets manager
    response = secrets_client.get_secret_value(SecretId=f"/{env_name}/DjangoSecretKey")
    config["environment"].append(
        {"name": "DJANGO_SECRET_KEY", "value": response["SecretString"]}
    )
    # Get the name of the secret containing database secrets from SSM
    response = ssm_client.get_parameter(Name=f"/{env_name}/DatabaseSecretNameParam")
    ...
    config["environment"].append({"name": "DB_HOST", "value": db_secrets["host"]})
    config["environment"].append({"name": "DB_PORT", "value": str(db_secrets["port"])})
    config["environment"].append({"name": "DB_USER", "value": db_secrets["username"]})
    config["environment"].append(
        {"name": "DB_PASSWORD", "value": db_secrets["password"]}
    )
    config["environment"].append(
    ...
    if extra_env_vars:
        for var in extra_env_vars:
            ...
            config["environment"].append({"name": name, "value": value})
    return config
